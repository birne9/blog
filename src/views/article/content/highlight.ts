// 关键词高亮: 把正文(p/h2/h3/tip)里的框架 API 与模板语法包成 <code class="kw">, 供 v-html 渲染
// 输入文本先转义再替换, 代码块不做处理
...
// withBind 为 true 时先做 Vue 模板语法绑定遍(@事件/:绑定); React 无此语法, 跳过
function highlightKeywords(text: string, keywords: string[], withBind: boolean): string {
...
export function highlightVueKeywords(text: string): string {
    return highlightKeywords(text, VUE_KEYWORDS, true)
}

// 高亮 React 关键词(Hooks/API/JSX 属性)
export function highlightReactKeywords(text: string): string {
    return highlightKeywords(text, REACT_KEYWORDS, false)
}
